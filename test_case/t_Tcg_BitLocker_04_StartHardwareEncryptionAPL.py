...
logging = LoggerManager(log_dir="..\script_log\\",
                        log_name=f"t_Tcg_BitLocker_04_StartHardwareEncryptionAPL_{current_time}").get_logger()


class StartHardwareEncryptionAPL():
    ...
    def Run(self):
        try:
            ...
            logging.info("step 4: delay power off")
            port = 'COM4'  # 串口名称
            delay_seconds = 5
            self.apl_lib.delay_poweroff(delay_seconds=delay_seconds, port=port)
...
if __name__ == '__main__':
    if ctypes.windll.shell32.IsUserAnAdmin():
        ...
        sys.exit()
    test = StartHardwareEncryptionAPL()
    test.Run()
